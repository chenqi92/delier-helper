...
function barChart(w, h, colors, axis, grid, valCol, cats, series, fs) {
  ...
  const pad = { l: 34, r: 12, t: multi ? fs + 12 : 10, b: 26 }
  ...
  for (let g = 0; g <= 4; g++) {
    ...
    s += `<text x="${pad.l - 5}" y="${y + fs * 0.35}" font-size="${fs * 0.85}" fill="${valCol}" text-anchor="end">${Math.round(top * g / 4)}</text>`
  }
  const n = Math.max(cats.length, 1)
  const groupW = pw / n
  const bc = Math.max(series.length, 1)
  const bw = Math.min((groupW * 0.66) / bc, 46)
  cats.forEach((cat, ci) => {
    const gx = pad.l + groupW * ci + groupW / 2
    series.forEach((se, si) => {
      const v = se.values[ci] || 0
      const bh = ph * (v / top)
      const x = gx - (bc * bw) / 2 + si * bw
      const y = pad.t + ph - bh
      s += `<rect x="${x}" y="${y}" width="${bw - 2}" height="${Math.max(0, bh)}" rx="2" fill="${colors[si % colors.length]}"/>`
    })
    s += `<text x="${gx}" y="${pad.t + ph + fs + 4}" font-size="${fs * 0.9}" fill="${valCol}" text-anchor="middle">${cat}</text>`
  })
...
function lineChart(w, h, colors, axis, grid, valCol, cats, series, fs, area) {
  ...
  series.forEach((se, si) => {
    ...
    if (area && si === 0) {
      s += `<polygon points="${xOf(0)},${pad.t + ph} ${pts.join(' ')} ${xOf(n - 1)},${pad.t + ph}" fill="${col}" opacity="0.16"/>`
    }
    s += `<polyline points="${pts.join(' ')}" fill="none" stroke="${col}" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/>`
    cats.forEach((_, i) => { s += `<circle cx="${xOf(i)}" cy="${yOf(se.values[i])}" r="3" fill="#fff" stroke="${col}" stroke-width="2"/>` })
  })
